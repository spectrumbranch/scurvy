...
internals.createUser = function(params, callback) {
	...
	internals.generateNewHash(passwrd, function(err, hashcake) {
		...
		User.create({ userid: userid, email: email, salt: salt, hash: hash }).success(function(user) {
			...
			internals.generateMetastateHashkey(email, salt, function(err1, result) {
...
exports.createUser = internals.createUser
