...
Returns a new instance of Scurvy with config options applied if specified.
...
The `callback` is fired when the query is complete with method signature `callback(err, result)` where `result` is false if the match does not exist, otherwise it will be an object with fields containing the sequelize objects of the found user: `{ user: user, metastate: metastate }`
